...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, d_k:Optional[int]=None, d_v:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0., 
        ...
        if self.decomposition:
            ...
            self.model = MLP_backbone(chong_len=chong_len, jian_layers = jian_layers, nei_layers = nei_layers, c_in=c_in, context_window = self.context_window, target_window=target_window, patch_len=patch_len, stride=stride,
                                  max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                  n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout,
                                  dropout=dropout, act=act, key_padding_mask=key_padding_mask, padding_var=padding_var, 
                                  attn_mask=attn_mask, res_attention=res_attention, pre_norm=pre_norm, store_attn=store_attn,
        ...
        # #拼接
        # self.linear1 = nn.Linear(configs.pred_len * 2, configs.pred_len, bias=True)

    def forward(self, x): #（32，336，7） x: [Batch, Input length, Channel]

        ################### norm
        seq_last = x[:,-1:,:].detach() #（32，1，7）
        if self.norm_type == 'revin':
            x = self.revin_layer(x, 'norm')
        elif self.norm_type == 'seq_last':
            x = x - seq_last
        global_x = x #（32，336，7）
        local_x = x[:,-self.context_len:,:] #（32，336，7）


        ################### Encoder: global branch
        if self.global_model == 'Gconv':
            global_x = self.global_layer_Gconv(global_x, return_kernel=False) #(32,336,7)
            global_x = self.linear_seq_pred(global_x.permute(0, 2, 1)).permute(0, 2, 1) #(32,96,7)
        ...
        if self.decomposition:
            ...
            global_x = global_x.permute(0, 2, 1) #(32,7,336) (32,7,96)
            local_x = self.model(global_x)  #(32,7,96)
            local_x = local_x.permute(0, 2, 1)  #(32,96,7)


        ################### Decoder
        # global_x_channel = self.linear_channel_in(global_x) #(32,96,128)
        # local_x_channel = self.linear_channel_in(local_x) #(32,96,128)
        # output_channel_l = self.ff(self.decoder_channel(global_x_channel, local_x_channel, local_x_channel)) + local_x_channel #(32,96,128)
        # output_channel_g = self.ff(self.decoder_channel(local_x_channel, global_x_channel, global_x_channel)) + global_x_channel #(32,96,128)
        # output_channel_l = self.norm_channel(output_channel_l.permute(0, 2, 1)).permute(0, 2, 1) #(32,96,128)
        # output_channel_g = self.norm_channel(output_channel_g.permute(0, 2, 1)).permute(0, 2, 1) #(32,96,128)
        # output_channel = self.atten_bias * output_channel_l + (1 - self.atten_bias) * output_channel_g  #(32,96,128)
        # output_channel = self.ff(self.linear_channel_out(output_channel))   #(32,96,7)
        #
        # global_x_token = self.linear_token_in(global_x.permute(0, 2, 1)) #（32，7，512）
        # local_x_token = self.linear_token_in(local_x.permute(0, 2, 1)) #（32，7，512）
        # output_token_l = self.ff(self.decoder_token(global_x_token, local_x_token, local_x_token)) + local_x_token #（32，7，512）
        # output_token_g = self.ff(self.decoder_token(local_x_token, global_x_token, global_x_token)) + global_x_token #（32，7，512）
        # output_token_l = self.norm_token(output_token_l.permute(0, 2, 1)).permute(0, 2, 1) #（32，7，512）
        # output_token_g = self.norm_token(output_token_g.permute(0, 2, 1)).permute(0, 2, 1) #（32，7，512）
        # output_token = self.atten_bias * output_token_l + (1 - self.atten_bias) * output_token_g  #（32，7，512）
        # output_token = self.ff(self.linear_token_out(output_token).permute(0, 2, 1))  #（32，7，96）

        # output = self.TC_bias * output_channel + (1 - self.TC_bias) * output_token + self.global_bias * global_x + self.local_bias * local_x

        #根据权重加
        # output = self.global_bias * global_x + self.local_bias * local_x

        global_x = global_x.permute(0, 2, 1)
        output = self.local_bias * local_x + self.global_bias * global_x

        # # 拼接
        # local_x = local_x.permute(0, 2, 1)
        # global_x = global_x.permute(0, 2, 1)
        # output = torch.cat([global_x, local_x], dim=2)
        # output = self.linear1(output)
        # output = output.permute(0, 2, 1) #(32,96,7)
