...
class Model(nn.Module):
    ...
    def forward(self, x): #（32，336，7） x: [Batch, Input length, Channel]
        ...
        if self.decomposition:
            ...
            local_x = local_x.permute(0, 2, 1) #(32,7,336)
            local_x = self.model(local_x)  #(32,7,96)
            local_x = local_x.permute(0, 2, 1)  #(32,96,7)

        ################### Encoder: global branch
        if self.global_model == 'Gconv':
            global_x_1 = self.global_layer_Gconv(local_x, return_kernel=False)  # (32,336,7)
            # global_x_1 = self.linear_seq_pred(global_x_1.permute(0, 2, 1)).permute(0, 2, 1)  # (32,96,7)
        ...
        ################### 残差: global branch
        if self.global_model == 'Gconv':
            global_x_2 = self.global_layer_Gconv(global_x, return_kernel=False)  # (32,336,7)
            global_x_2 = self.linear_seq_pred(global_x_2.permute(0, 2, 1)).permute(0, 2, 1)  # (32,96,7)
        elif self.global_model == 'FNO':
            global_x = global_x.permute(0, 2, 1).unsqueeze(3)
            global_x = self.global_layer_FNO(global_x)
            global_x = self.linear_seq_pred(global_x.squeeze(2)).permute(0, 2, 1)
        ...
        output = self.global_bias * global_x_1 + self.local_bias * global_x_2
